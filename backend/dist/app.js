...
app.get('/api/summary', async (req, res) => {
    const [rows] = await db.query('SELECT SUM(CASE WHEN type=\'income\' THEN amount ELSE 0 END) AS totalIncome, SUM(CASE WHEN type=\'expense\' THEN amount ELSE 0 END) AS totalExpense FROM expenses');
...
app.post('/add', async (req, res) => {
    ...
    await db.execute(`INSERT INTO expenses (user_id, category_id, type, amount, date, note)
